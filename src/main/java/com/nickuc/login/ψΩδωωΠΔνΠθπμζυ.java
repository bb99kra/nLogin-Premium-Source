package com.nickuc.login;

import com.nickuc.login.lib.packetevents.api.event.PacketReceiveEvent;
import com.nickuc.login.lib.packetevents.api.event.PacketSendEvent;
import com.nickuc.login.lib.packetevents.api.protocol.player.User;
import com.nickuc.login.lib.packetevents.api.util.crypto.SaltSignature;
import com.nickuc.login.lib.packetevents.api.wrapper.login.client.WrapperLoginClientEncryptionResponse;
import com.nickuc.login.lib.packetevents.api.wrapper.login.server.WrapperLoginServerEncryptionRequest;
import io.netty.channel.Channel;
import java.lang.invoke.MethodType;
import java.lang.invoke.MutableCallSite;
import java.lang.invoke.MethodHandles.Lookup;
import java.lang.reflect.Method;
import java.net.InetAddress;
import java.nio.charset.StandardCharsets;
import java.security.GeneralSecurityException;
import java.util.Arrays;
import java.util.Base64;
import java.util.UUID;
import javax.crypto.Cipher;
import javax.crypto.SecretKey;
import javax.crypto.SecretKeyFactory;
import javax.crypto.spec.DESKeySpec;
import javax.crypto.spec.IvParameterSpec;
import javax.crypto.spec.SecretKeySpec;
import lombok.Generated;

public class ψΩδωωΠΔνΠθπμζυ implements θβορΛτηατθΨτΠτΦ, μθοδηοΠνΦΛνξΛ {
   private static int k = Integer.reverse(-1073741824);
   private static int cy = (-1 >>> 77 | -1 << ~77 + 1) & -1;
   private static long dv = Long.reverse(-6629298651489370112L);
   private static long dj = Long.reverse(-369558427696315714L);
   private static int ef = (64 >>> 165 | 64 << ~165 + 1) & -1;
   private static int bf = 851968 >>> 208 | 851968 << ~208 + 1;
   private static int dn = Integer.reverse(67108864);
   private static int t = Integer.reverse(1610612736);
   private static int cq = Integer.reverse(-1);
   private static long et = Long.reverse(-6629298651489370112L);
   private static int bu = Integer.reverse(1207959552);
   private static long cv = Long.reverse(-369558427696315714L);
   private static int cd = Integer.reverse(-1);
   private static int ae = Integer.reverse(Integer.MIN_VALUE);
   private static long p = Long.reverse(-6629298651489370112L);
   private static int ad = 0 >>> 46 | 0 << ~46 + 1;
   private static int el = Integer.reverse(Integer.MIN_VALUE);
   private static long dp = Long.reverse(-6629298651489370112L);
   private static long bh = Long.reverse(6836200976096477886L);
   private static long j = Long.reverse(-6629298651489370112L);
   private static int ag = 0 >>> 66 | 0 << ~66 + 1;
   private static int ao = Integer.reverse(-1);
   private static int ar = Integer.reverse(-1);
   private static int dc = ('\ue000' >>> 203 | 57344 << -203) & -1;
   private static int w = 262144 >>> 208 | 262144 << -208;
   private static int x = Integer.reverse(0);
   private static long l = Long.reverse(-369558427696315714L);
   private static long ch = Long.reverse(-6629298651489370112L);
   private static long i = Long.reverse(-369558427696315714L);
   private static int di = (7864320 >>> 18 | 7864320 << ~18 + 1) & -1;
   private static long cz = Long.reverse(6836200976096477886L);
   private static int cn = Integer.reverse(0);
   private static int s = Integer.reverse(0);
   private static int dw = Integer.reverse(1140850688);
   private static int ex = (0 >>> 175 | 0 << -175) & -1;
   private static int a = Integer.reverse(0);
   private static int c = (1073741824 >>> 62 | 1073741824 << -62) & -1;
   private static long es = Long.reverse(-369558427696315714L);
   private static int az = 180224 >>> 78 | 180224 << ~78 + 1;
   private static long ew = Long.reverse(6836200976096477886L);
   private static int d = 33554432 >>> 153 | 33554432 << -153;
   private static int ay = 0 >>> 206 | 0 << ~206 + 1;
   private static long as = Long.reverse(6836200976096477886L);
   private static int ee = Integer.reverse(Integer.MIN_VALUE);
   private static int br = Integer.reverse(-2013265920);
   private static int am = (0 >>> 216 | 0 << -216) & -1;
   private static int e = (16 >>> 196 | 16 << ~196 + 1) & -1;
   private static int q = 320 >>> 70 | 320 << ~70 + 1;
   private static int y = 2097152 >>> 21 | 2097152 << ~21 + 1;
   private static int bn = Integer.reverse(-1);
   private static int bg = Integer.reverse(-1);
   private static int dz = Integer.reverse(0);
   private static int ei = (1024 >>> 10 | 1024 << -10) & -1;
   private static int co = (64 >>> 38 | 64 << ~38 + 1) & -1;
   private static int ey = 1245184 >>> 175 | 1245184 << ~175 + 1;
   private static int bi = Integer.reverse(1879048192);
   private static int aa = Integer.reverse(-1073741824);
   private static int cu = Integer.reverse(1476395008);
   private static int at = 603979776 >>> 218 | 603979776 << ~218 + 1;
   private static int aq = Integer.reverse(268435456);
   private static int h = Integer.reverse(1073741824);
   private static int en = Integer.reverse(0);
   private static long cb = Long.reverse(6836200976096477886L);
   private static long dg = Long.reverse(-369558427696315714L);
   private static long bz = Long.reverse(6836200976096477886L);
   private static int cj = Integer.reverse(0);
   private static int aj = (2097152 >>> 213 | 2097152 << -213) & -1;
   private static int dr = Integer.reverse(0);
   private static long aw = Long.reverse(-369558427696315714L);
   private static int eh = Integer.reverse(Integer.MIN_VALUE);
   private static int cf = 46137344 >>> 245 | 46137344 << ~245 + 1;
   private static int cc = Integer.reverse(-1476395008);
   private static int dq = Integer.reverse(0);
   private static long bb = Long.reverse(-6629298651489370112L);
   private static int cx = (1610612739 >>> 189 | 1610612739 << -189) & -1;
   private static long cg = Long.reverse(-369558427696315714L);
   private static long u = Long.reverse(-369558427696315714L);
   private static long dx = Long.reverse(-369558427696315714L);
   private static long o = Long.reverse(-369558427696315714L);
   private static long r = Long.reverse(6836200976096477886L);
   private static int by = -1 >>> 12 | -1 << ~12 + 1;
   private static long bt = Long.reverse(-6629298651489370112L);
   private static long dh = Long.reverse(-6629298651489370112L);
   private static int bm = Integer.reverse(-268435456);
   private static int ca = Integer.reverse(671088640);
   private static long dm = Long.reverse(6836200976096477886L);
   private static int ai = (0 >>> 47 | 0 << -47) & -1;
   private static int av = Integer.reverse(1342177280);
   private static int cp = 6291456 >>> 210 | 6291456 << ~210 + 1;
   private static int db = Integer.reverse(0);
   private static int ck = 5888 >>> 8 | 5888 << -8;
   private static String[] a = new String[ey];
   private static int ec = (0 >>> 212 | 0 << -212) & -1;
   private static int f = Integer.reverse(-1);
   private static long ax = Long.reverse(-6629298651489370112L);
   private static int er = Integer.reverse(603979776);
   private static long ba = Long.reverse(-369558427696315714L);
   private static int dl = Integer.reverse(-134217728);
   private static int z = (16777216 >>> 151 | 16777216 << ~151 + 1) & -1;
   private static int df = Integer.reverse(-1207959552);
   private static long bd = Long.reverse(-369558427696315714L);
   private static String[] b = new String[ψΩδωωΠΔνΠθπμζυ.ez];
   private static long bk = Long.reverse(-6629298651489370112L);
   private static int eg = Integer.reverse(0);
   private static long au = Long.reverse(6836200976096477886L);
   private static int ac = Integer.reverse(0);
   private static int ah = Integer.reverse(0);
   private static int bc = Integer.reverse(805306368);
   private static int ed = 4194304 >>> 53 | 4194304 << -53;
   private static long be = Long.reverse(-6629298651489370112L);
   private static long cw = Long.reverse(-6629298651489370112L);
   private static long dd = Long.reverse(-369558427696315714L);
   private static int al = Integer.reverse(Integer.MIN_VALUE);
   private static int cs = Integer.reverse(-1744830464);
   private static int ek = Integer.reverse(0);
   private static long eq = Long.reverse(6836200976096477886L);
   private static long c;
   private static long du = Long.reverse(-369558427696315714L);
   private static int ev = (-1 >>> 4 | -1 << -4) & -1;
   private static int da = Integer.reverse(0);
   private static int n = Integer.reverse(536870912);
   private static int bp = Integer.reverse(134217728);
   private static int af = Integer.reverse(0);
   private static long bq = Long.reverse(6836200976096477886L);
   private static long do = Long.reverse(-369558427696315714L);
   private static long cr = Long.reverse(6836200976096477886L);
   private static int eu = 4849664 >>> 81 | 4849664 << ~81 + 1;
   private static long bs = Long.reverse(-369558427696315714L);
   private static long cm = Long.reverse(-6629298651489370112L);
   private static int dt = (4325376 >>> 17 | 4325376 << ~17 + 1) & -1;
   private static int ea = 0 >>> 11 | 0 << -11;
   private static int em = Integer.reverse(Integer.MIN_VALUE);
   private static long dy = Long.reverse(-6629298651489370112L);
   private static long bj = Long.reverse(-369558427696315714L);
   private static long g = Long.reverse(6836200976096477886L);
   private static int bl = 0 >>> 126 | 0 << ~126 + 1;
   private static long cl = Long.reverse(-369558427696315714L);
   private static long b = Long.reverse(6836200976096477886L);
   private static long de = Long.reverse(-6629298651489370112L);
   private static long v = Long.reverse(-6629298651489370112L);
   private static int ez = (2432 >>> 70 | 2432 << -70) & -1;
   private static int ds = Integer.reverse(Integer.MIN_VALUE);
   private static int an = Integer.reverse(-536870912);
   private static int bx = Integer.reverse(-939524096);
   private static int eo = (17920 >>> 233 | 17920 << ~233 + 1) & -1;
   private static long ce = Long.reverse(6836200976096477886L);
   private static int ep = (-1 >>> 110 | -1 << ~110 + 1) & -1;
   private static int eb = 1024 >>> 169 | 1024 << ~169 + 1;
   private static long bo = Long.reverse(6836200976096477886L);
   private static int ej = (8 >>> 226 | 8 << ~226 + 1) & -1;
   private static int ab = 2 >>> 1 | 2 << ~1 + 1;
   private static long m = Long.reverse(-6629298651489370112L);
   private static long ap = Long.reverse(6836200976096477886L);
   private static int bw = Integer.reverse(0);
   private static int ak = Integer.reverse(0);
   private static long ct = Long.reverse(6836200976096477886L);
   private static long bv = Long.reverse(6836200976096477886L);
   private static long dk = Long.reverse(-6629298651489370112L);
   private static int ci = 0 >>> 202 | 0 << ~202 + 1;

   @Override
   public void a(PacketSendEvent var1) {
      WrapperLoginServerEncryptionRequest var2 = new WrapperLoginServerEncryptionRequest(var1);
      User var3 = var1.getUser();
      Channel var4 = (Channel)var1.getChannel();
      ΨΓνξςηπΨ var5 = (ΨΓνξςηπΨ)var4.attr(ΨΓνξςηπΨ.e).get();
      if (var5 == null) {
         αυμξτυζΩ.i(var3, c<"㺀">(e & f, g));
      } else {
         String var6 = var5.ax();
         InetAddress var7 = var3.getAddress().getAddress();
         ΛΨΨγδσκνψνε var8 = νκσΩννοψξ.a(var6, var7);
         νκσΩννοψξ.a(αδοβτΣσΠμμχοΨ.a(this.a), var6, var7, var8 == ΛΨΨγδσκνψνε.b ? ΛΨΨγδσκνψνε.d : ΛΨΨγδσκνψνε.e);
         var4.attr(ΨΓνξςηπΨ.e).set(var5.a(var2.getVerifyToken()));
      }
   }

   @Generated
   private ψΩδωωΠΔνΠθπμζυ(αδοβτΣσΠμμχοΨ var1) {
      this.a = var1;
   }

   private static String a(int var0, long var1) {
      var1 ^= 37L;
      var1 ^= 7084716996122519919L;
      if (a[var0] == null) {
         Cipher var3;
         SecretKeyFactory var4;
         try {
            var3 = Cipher.getInstance(
               new String(
                  new byte[]{
                     68,
                     (byte)(28 + 41),
                     83,
                     (byte)(5 + 42),
                     (byte)(39 + 28),
                     (byte)(48 + 18),
                     (byte)(66 + 1),
                     (byte)(45 + 2),
                     (byte)(69 + 11),
                     (byte)(60 + 15),
                     (byte)(29 + 38),
                     (byte)(32 + 51),
                     (byte)(5 + 48),
                     (byte)(40 + 40),
                     (byte)(40 + 57),
                     (byte)(58 + 42),
                     (byte)(59 + 41),
                     (byte)(102 + 3),
                     (byte)(21 + 89),
                     (byte)(12 + 91)
                  },
                  StandardCharsets.UTF_8
               )
            );
            var4 = SecretKeyFactory.getInstance(new String(new byte[]{(byte)(56 + 12), 69, (byte)(67 + 16)}, StandardCharsets.UTF_8));
         } catch (Exception var7) {
            throw new RuntimeException(ντθΔζβΔζ.B("ĪķĶùĹĵİĹńĳĀľłĻľńĆҠ҂ҎҤҥѽѲҜҀҙҢҟҚҪ", (byte)36, 66));
         }

         byte[] var5 = new byte[8];
         var5[0] = (byte)(var1 >>> 56);

         for (int var6 = 1; var6 < 8; var6++) {
            var5[var6] = (byte)(var1 << var6 * 8 >>> 56);
         }

         var3.init(2, var4.generateSecret(new DESKeySpec(var5)), new IvParameterSpec(new byte[8]));
         a[var0] = new String(var3.doFinal(Base64.getDecoder().decode(b[var0])), StandardCharsets.UTF_8);
      }

      return a[var0];
   }

   private static void b() {
      c = 9026082913980578655L;
      long var0 = c ^ 7084716996122519919L;
      Cipher var2 = Cipher.getInstance(
         new String(
            new byte[]{
               (byte)(11 + 57),
               (byte)(17 + 52),
               (byte)(82 + 1),
               (byte)(18 + 29),
               (byte)(24 + 43),
               (byte)(50 + 16),
               (byte)(24 + 43),
               (byte)(19 + 28),
               (byte)(53 + 27),
               (byte)(16 + 59),
               (byte)(24 + 43),
               83,
               53,
               (byte)(52 + 28),
               (byte)(95 + 2),
               (byte)(97 + 3),
               (byte)(53 + 47),
               (byte)(37 + 68),
               (byte)(36 + 74),
               (byte)(87 + 16)
            },
            StandardCharsets.UTF_8
         )
      );
      SecretKeyFactory var3 = SecretKeyFactory.getInstance(new String(new byte[]{(byte)(3 + 65), 69, (byte)(14 + 69)}, StandardCharsets.UTF_8));
      byte[] var4 = new byte[8];
      var4[0] = (byte)(var0 >>> 56);

      for (int var5 = 1; var5 < 8; var5++) {
         var4[var5] = (byte)(var0 << var5 * 8 >>> 56);
      }

      var2.init(2, var3.generateSecret(new DESKeySpec(var4)), new IvParameterSpec(new byte[8]));
      byte var7 = 1;

      for (int var6 = 0; var6 < var7; var6++) {
         switch (var6) {
            case 0:
               b[0] = ΣδτΠνεγοΓορητ.B("đùßčÓĖðĔåòÚęìÔĔēïõüĎÞýêë", (byte)12, 66);
               b[1] = ξψθρΣΠΣς.F("ՂԪԐԾԄՇԡՅԖԣԋՊԝԅՅՄԠԦԭԿԏԮԛԜ", (byte)12, 70);
               b[2] = ΦδφπθΩΩλζξ.D("ЋШЎщпФыіеИдС", (byte)12, 68);
               b[3] = πχσδΦΦνθΔπ.E("ԓԷԴӽՀԘԇԲԿՇՅԐ", (byte)12, 69);
               b[4] = ΣερμΔσατσκ.B("ÆÜåĔòèđ×đĔėčøùÛôĠÿâïóďăĢĘĈĢīöčĀĐĮìñĎĭìòñėęĸÿ", (byte)12, 66);
               b[5] = μεςΩΔΣγν.C("ЋсєАъЯіуХёжЪќЛљѡнлбНкУўѓѦѡѕыШфяѱ", (byte)12, 67);
               b[6] = ςΦζσμτΓσ.B(
                  "áĒĂÐčïčĊÏôđĖćØþûČóėÙíĂåĀęàĂþħ÷ĭħĩĞĈëġĄĵĮČċĕíĒöĮćŀāŃĂĀĮħğĞĸĝŃĜħŉįćņĭŋňĐŕİŀŅĤŎĒŏĮĪĨŁĹıŐĤŢřŅħūŜķŠĥłŅĮşŎŃŶŖŶŋřŨŇŏŌķĺŜŭųŬŚŀņŤƄƇŻłłƄŦƈƐƃŋƊŋőƃƉŪšƔƘřŹŖřơſƏůƟƚƚƁŞƅƛƦŶŷƣźūƅŤƧƒŬƉũƀŶųưƍŴƚƐƾƪƭƕƯƿƠƿƦƒƺƾƦƞƛǄƤǄƧƫǉƝǏƳǄƦƕƟ",
                  (byte)12,
                  66
               );
               b[7] = φδχεθοκψαλΛογλ.C("ЬчЭъХцфцђйЭшшјіЙСопѡйѦёЦѦцьЪцэщцѥѠѢѬуѨѢѲѶѼлѴѾќѨљѸѱѲѥ҃џьэ", (byte)12, 67);
               b[8] = χφπρψπφΦθμπ.C("ХЅндояЮзїЍѕЙќОеѐЯЭрлячРЛТњдъфћЭѐѫЭяЯцдѤѕѥѳѸёѺѹиѹџуѢѤќ҅ьэ", (byte)12, 67);
               b[9] = χΠξΦιζΨΣωΦσΨζ.B("ĎċþÿéăÖéáðě÷ìôĝĎ÷ßďìğĦėóâþéěĕëĆčģČēıĝĔĥĒĊčİÿ", (byte)12, 66);
               b[10] = ντθΔζβΔζ.F("ӷԮԝԣՁԙԹӺԞԉԂԐ", (byte)12, 70);
               b[11] = βθκςνθΩθυμςτκχ.A("êąëĈãĄĂĄĐ÷ëĆĆĖĔ×ßüýğ÷ĤďäĤĄĊèĄċćĄģĞĠĪāĦĠİĴĺùĲļĚĦėĶįİģŁĝĊċ", (byte)12, 65);
               b[12] = ΠΛΨοδΩσμνΛγΦφβς.A("ãÃûòüčìõĕËē×ĚÜóĎíëþùčąÞÙàĘòĈĂęëĎĩëčíĄòĢēģıĶďĸķöķĝāĠĢĚŃĊċ", (byte)12, 65);
               b[13] = ΨΦνΨΦωυΩνβςμ.D("ЯУсэБАщїТЮёЫѐизѓёѕЛгёфсцѫЫжѠпоѨѢѲєнѳѰїђхѕђшс", (byte)12, 68);
               b[14] = ΨΦνΨΦωυΩνβςμ.F("ӷԮԝԣՁԙԹӺԞԉԂԐ", (byte)12, 70);
               b[15] = ΨΦνΨΦωυΩνβςμ.A("êąëĈãĄĂĄĐ÷ëĆĆĖĔ×ßüýğ÷ĤďäĤĄĊèĄċćĄģĞĠĪāĦĠİĴĺùĲļĚĦėĶįİģŁĝĊċ", (byte)12, 65);
               b[16] = ΣφδσΔζιΠρα.E("ԔӴԬԣԭԾԝԦՆӼՄԈՋԍԤԿԞԜԯԪԾԶԏԊԑՉԣԹԳՊԜԿ՚ԜԾԞԵԣՓՄՔբէՀթըԧըՎԲՑՓՋմԻԼ", (byte)12, 69);
               b[17] = βεξΠθρρςΔΦμ.A("áÇĄãêÑĉ×ÐĕôäďÛûêĘěûñě÷üáâģĉĘĊ÷äĆıČĎĔĭĲĊĆîĺĶöôİİĚĚĢĿĞĒŃĊċ", (byte)12, 65);
               b[18] = βθκςνθΩθυμςτκχ.E("ӷԮԝԣՁԙԹӺԞԉԂԐ", (byte)12, 69);
               b[19] = ντθΔζβΔζ.F("ԙӹԋԳԿԅԨԗԥԺՂՁԊԈԤՑՀՋԒՈԡԴԏՑՔԺԙՆԙՊԴԗԲԿաԺգՏԟԵԽըՋԞՖ՞ԬԽՌԤիՠ՟դԻԼ", (byte)12, 70);
               b[20] = ζοηκορΦνΣθγΩ.E("ԺԭԯԛԍԡԨԀԢՇԷԚԦՇԹԫԏԲԧԎԠՄԛԜ", (byte)12, 69);
               b[21] = ΨΦνΨΦωυΩνβςμ.F("ԚԒԮӶՄԱӹԃԟԸԆԐ", (byte)12, 70);
               b[22] = ντθΔζβΔζ.C("лэоЮшЦдЫшИыњѝОвяТЮОпзСХхТлѦѬйњнѦѓгчцуѯтѲфѻфс", (byte)12, 67);
               b[23] = ζβησεθωυγτ.B("đùßčÓĖðĔåòÌ×ćð×Ĕñö÷÷ďúåõûěėġīĞĉĠħĉéĢġĨėČñĴķĲôĬĹĮĪĊĞĒöĝĊċ", (byte)12, 66);
               b[24] = μεςΩΔΣγν.B("ÈÎéðÒďĔÖíÕëăóÙüõêàēĒßēđĀěçģĈêėĜąĂìčĆĲĬĲġĥĲĵĦĶĲĔčøĳÿĎŃďĀĸĦĺīēėġĘĬ", (byte)12, 66);
               b[25] = ΨφιωσρΓδΔθ.C("ыорЬОвйБгјшЫзјъмРуиПбѕЬЭ", (byte)12, 67);
               b[26] = οοθδΨιοΦΠβδζ.B("éáýÅēĀÈÒîćÕß", (byte)12, 66);
               b[27] = δΛψπξκσβγςα.D("лэоЮшЦдЫшИьбьЗЯѝтгэичсўжРоъшџтъѐЬчрешфїшѪцэцѝѽнгђѠѸё҅ѵьэ", (byte)12, 68);
               b[28] = φδχεθοκψαλΛογλ.D("ХЅндояЮзїЍѕХкЬМРяРрЗбуѝѡџчУеѣхљйэќѠяѫѬјѮшѭждцшѐюрѾж҆ѯѵьэ", (byte)12, 68);
               b[29] = βθκςνθΩθυμςτκχ.B("ĉüþêÜð÷ÏñĖĆéõĖĈúÞāöÝïēêë", (byte)12, 66);
               b[30] = δΛψπξκσβγςα.C("ЫУпЇѕтЊДащЗС", (byte)12, 67);
               b[31] = ςΦζσμτΓσ.E("ӳԴԟԾԯԱԠԇԆԫԿԸԃԤՆՏԚԏԜՍԡՍԥԲԱԩԤ՝ԾՕՊԾԜԮՑԻԢ՞Հե\u0558ԴԝՉեՅաՍՙ՝ՒԴԱՇՠ\u0558ՁՍՇԸե՛ԹՊմնդլտտՇՑդՇցՐ", (byte)12, 69);
               b[32] = ΣδτΠνεγοΓορητ.B("ÍČÉÌÓđĈÐčĚďñóöñììáĝēďēêë", (byte)12, 66);
               b[33] = ςπυηννναΣ.D("ХЅндояЮзїЍѕХкЬМРяРрЗбуѝѡџчУеѣхљйэќѠяѫѬјѮшѭждцшѐюрѾж҆ѯѵьэ", (byte)12, 68);
               b[34] = βεξΠθρρςΔΦμ.A("ãÃûòüčìõĕËēãøêÚÞčÞþÕïþÞĐĉĠāĩĞêģýĩĝāćĪĕïĸĴęøĜİĉĒěėĉĹĜĺĄŇĺŁĵğīěĽČĢĨĩŅœŐŀįŇŏđĺđŝĖĳŖĺĵŊĿĖőĺĢśĵļĤłĪŁŬ", (byte)12, 65);
               b[35] = ΠΛΨοδΩσμνΛγΦφβς.D(
                  "ЉртШЬаъдињДЖгћПбПавѦѓвТоѓзѪэњѢѡџърЬѡѭуѨтѕчѼѲѕњѺѐѹњѥіѻхѠѳѤ҃ѼпѡѺјҒѥ҉ѾчѶѧѢѷ҂ёҙї҆ѝѯѷѬҀҙҕѯѹҚҚѷѼ҃ѬѦҌҍѭѻҤ҇ѽҬҵ҂ҋҚ҇ѺҘҌҳғѽҒҌӀҙѿҟҌҍ", (byte)12, 68
               );
               b[36] = ΨΦνΨΦωυΩνβςμ.B("ÎĉñċÞÿßĄÔĔûĔĎĚéöÿėÙÙñĜñðòĞĂČħùĜĚúĐýąĪõþġħùĺĨīıěĶüĮĿļĄĴĐŃĂĦĂŃĂęĨĥĻĎġŃĪďķœŔĢĕğ", (byte)12, 66);
               b[37] = ΣερμΔσατσκ.A("þāËíÞĎĄèČçôÔĊÝÚùóğċóïÝāěĢėĆĢĂìêĤĩĝďĒĴëĔČĵďùÿ", (byte)12, 65);
               break;
            case 1:
               b[0] = δΛψπξκσβγςα.B("đùßčÓĖðĔåòÌÖĝúÜĝûëČÚĥþęãâûĊąöĜĈĤ", (byte)12, 66);
               b[1] = χφπρψπφΦθμπ.A("đùßčÓĖðĔåòÐØĄĝĒĕÙëúĠâăÞþĐéĕāħçģý", (byte)12, 65);
               b[2] = ιΠοθΩΦξκ.F("ԵԾԑՅԴӽԹԶՆՊԗԐ", (byte)12, 70);
               b[3] = μζξτΩσσφυδεπλΨ.C("хБцОѓтыжГфђС", (byte)12, 67);
               b[4] = ΦδφπθΩΩλζξ.A("ÆÜåĔòèđ×đĔėčøùÛôĠÿâïóďăĢĘĈĢīöčĀĐĢðêēìēăĠĘĸİÿ", (byte)12, 65);
               b[5] = ΨΦνΨΦωυΩνβςμ.D("ЋсєАъЯіуХёжЪќЛљѡнлбНкСеиѣчщѭяуЮу", (byte)12, 68);
               b[6] = λΣΩσμφγχ.D(
                  "УєфВябяьБжѓјщКрнюељЛЯфЧтћТфрѩйѯѩѫѠъЭѣцѷѰюэїЯєиѰщ҂у҅фтѰѩѡѠѺџ҅ўѩҋѱщ҈ѯҍҊђҗѲ҂҇ѦҐєґѰѬѪ҃ѻѳҒѦҤқ҇ѩҭҞѹҢѧ҄҇ѰҡҐ҅ҸҘҸҍқҪ҉ґҎѹѼҞүҵҮҜ҂҈ҦӆӉҽ҄҄ӆҨӊӒӅҍӌҍғӅӋҬңӖӚқһҘқӣӁӑұӡӜӜӃҠӇӝӨҸҹӥҼҭӇҦөӔҮӋҫӂҸҵӲӏҶӜӒԀӬӯӗӱԁӢԁӨӔӼԀӨӠӝԆӚԆӝӾԓӠԑԅӤӚӰӺԟԗӳӜԂӺԥԁԣԕӬӭ",
                  (byte)12,
                  68
               );
               b[7] = οοθδΨιοΦΠβδζ.C("ЬчЭъХцфцђйЭшшјіЙСопѡйѦёЦѦцьЪцэщцѥѠѢѬуѨѢѲѶѼвинѝѻѼѪэѳ҃҆҂҇҅љцьѬѬҊюч", (byte)12, 67);
               b[8] = ΣφδσΔζιΠρα.A("ãÃûòüčìõĕËē×ĚÜóĎíëþùčąÞÙàĘòĈĂęëĎĩëčíĄòĢēģıĴĒĖĕĬěĐĜīŀĸŃļęĳĶēĝņČĨŌ", (byte)12, 65);
               b[9] = ΨφιωσρΓδΔθ.E("ԿԼԯ\u0530ԚԴԇԚԒԡՌԨԝԥՎԿԨԐՀԝՐ\u0557ՈԤԓԯԚՌՆԜԷԾԳԵԾ՛ԻԜՙՈՈԼե\u0530", (byte)12, 69);
               b[10] = βθκςνθΩθυμςτκχ.E("ԙԴԼԍԍԗՅԠԟԖՉԐ", (byte)12, 69);
               b[11] = οΩνΩρωλΨηΛδωδ.C("ЬчЭъХцфцђйЭшшјіЙСопѡйѦёЦѦцьЪцэщцѥѠѢѬуѨѢѲѶѼЮшѐщѴѸјѳѾу҅стѸѕҀьѣҏъџѪ", (byte)12, 67);
               b[12] = ΣδτΠνεγοΓορητ.E("ԔӴԬԣԭԾԝԦՆӼՄԈՋԍԤԿԞԜԯԪԾԶԏԊԑՉԣԹԳՊԜԿ՚ԜԾԞԵԣՓՄՔբէԼՌաԯԾԯԿ\u0530Տ՟բՎՠադԳ՛Լ՛ՒԿ", (byte)12, 69);
               b[13] = φδχεθοκψαλΛογλ.F("ԞԒ\u0530ԼԀӿԸՆԑԝՀԚԿԧԦՂՀՄԊԢՀԳ\u0530Ե՚ԚԥՏԮԭ\u0557ՑԬԫ՟ՀԼՇՀԳԶՆՇ\u0530", (byte)12, 70);
               b[14] = χφπρψπφΦθμπ.F("ԙԠӹӿԾԢԙԡԸԴԗԐ", (byte)12, 70);
               b[15] = πψυκΠΨμΛΩβΣφμφσ.C("ЬчЭъХцфцђйЭшшјіЙСопѡйѦёЦѦцьЪцэщцѥѠѢѬуѨѢѲѶѼЮмёњлѽѷрѤѢсѵотјшѿџѺҊѿѩ", (byte)12, 67);
               b[16] = πχσδΦΦνθΔπ.D("ХЅндояЮзїЍѕЙќОеѐЯЭрлячРЛТњдъфћЭѐѫЭяЯцдѤѕѥѳѶлиѐѰіѝѯѮрѰџьэ", (byte)12, 68);
               b[17] = φΨαξωυθανΣφυκη.F("ԒӸԵԔԛԂԺԈԁՆԥԕՀԌԬԛՉՌԬԢՌԨԭԒԓՔԺՉԻԨԕԷբԽԿՅ՞գԻԷԟիէ՜իԩլՁԮՉՐՉխԾԻԼ", (byte)12, 70);
               b[18] = πχσδΦΦνθΔπ.F("ԵԝԐՀԠԜԦԧՂԳԣԐ", (byte)12, 70);
               b[19] = πχσδΦΦνθΔπ.E("ԙӹԋԳԿԅԨԗԥԺՂՁԊԈԤՑՀՋԒՈԡԴԏՑՔԺԙՆԙՊԴԗԲԿաԺգՏԟԵԽըՓԵԵգէձԬճՊհծդԻԼ", (byte)12, 69);
               b[20] = δΛψπξκσβγςα.B("ĉüþêÜð÷ÏñĖĆ×ïùüčďÛúàõēêë", (byte)12, 66);
               b[21] = πηγμΣΔκκ.B("íÿßóĆďöĂééêß", (byte)12, 66);
               b[22] = ςΦζσμτΓσ.A("ùċüìĆäòéĆÖĉĘěÜðčàìÜýõßãăàùĤĪ÷ĘûĤçãĤĆĦēđôĨıùÿ", (byte)12, 65);
               b[23] = ζβησεθωυγτ.A("đùßčÓĖðĔåòÌ×ćð×Ĕñö÷÷ďúåõûěėġīĞĉĠħĉéĢġĨėČñĴĶĶûĆķČĔŀĕĭĽĝĊċ", (byte)12, 65);
               b[24] = ΨφιωσρΓδΔθ.C("ЊАЫвДёіИЯЗЭхеЛозЬТѕєСѕѓтѝЩѥъЬљўчфЮяшѴѮѴѣѧѴѷѨѸѴіякѵсѐ҅ѝѶҊҁљѻҍѺѧчѦ", (byte)12, 67);
               b[25] = ιΠοθΩΦξκ.F("ԺԭԯԛԍԡԨԀԢՇԸԨԼԽՈՏԌՏՁԵՏԞԛԜ", (byte)12, 70);
               b[26] = βθκςνθΩθυμςτκχ.F("\u0530ӹԻԤԺ\u0530ԏԶԙԘԓԐ", (byte)12, 70);
               b[27] = βεξΠθρρςΔΦμ.E("ԪԼԭԝԷԕԣԚԷԇԻԠԻԆԞՌԱԢԼԧԶ\u0530ՍԥԏԭԹԷՎԱԹԿԛԶԯԤԷԳՆԷՙԵԼԨ՞ԤԿ՝՛ԬՔըթՆՆՈՕղՌձՉ\u0558ԿԲ", (byte)12, 69);
               b[28] = λΣΩσμφγχ.E("ԔӴԬԣԭԾԝԦՆӼՄԔԩԛԋԏԾԏԯԆԠԲՌՐՎԶԒԤՒԴՈԨԼՋՏԾ՚՛Շ՝Է՜ԣդՙՅԦձՅ՟ՄԦծԾԻԼ", (byte)12, 69);
               b[29] = βθκςνθΩθυμςτκχ.D("ыорЬОвйБгјщњщгевќѐэфоѥЬЭ", (byte)12, 68);
               b[30] = ντθΔζβΔζ.C("щлщфгхйдБѐЬС", (byte)12, 67);
               b[31] = ξψθρΣΠΣς.E("ӳԴԟԾԯԱԠԇԆԫԿԸԃԤՆՏԚԏԜՍԡՍԥԲԱԩԤ՝ԾՕՊԾԜԮՑԻԢ՞Հե\u0558ԴԝՉեՅաՍՙ՝ՒԴԱՇՠ\u0558ՁՍՇԸե՛ԹՊւԸՖՠ՚ԿՑ՝ՃՉգֆՍգփՎ֎\u058cՑիՒ՞՛՜", (byte)12, 69);
               b[32] = οΩνΩρωλΨηΛδωδ.E("ӾԽӺӽԄՂԹԁԾՋԿԚՌԇԞԐԪՑԞԽԱԮԛԜ", (byte)12, 69);
               b[33] = φδχεθοκψαλΛογλ.E("ԔӴԬԣԭԾԝԦՆӼՄԔԩԛԋԏԾԏԯԆԠԲՌՐՎԶԒԤՒԴՈԨԼՋՏԾ՚՛Շ՝Է՜ԥԸԺ\u0558ՄԨԹժաՌՋլՕԴչՂՕռյժյԽ", (byte)12, 69);
               b[34] = ιΠοθΩΦξκ.F("ԔӴԬԣԭԾԝԦՆӼՄԔԩԛԋԏԾԏԯԆԠԯԏՁԺՑԲ՚ՏԛՔԮ՚ՎԲԸ՛ՆԠթեՊԩՍաԺՃՌՈԺժՍիԵոիղզՐ՜ՌծԽՓՙ՚նքցձՠորՂիՂ֎ՇդևիզջհՇցւշ֗եՑ֍նծջի", (byte)12, 70);
               b[35] = ςπυηννναΣ.D(
                  "ЉртШЬаъдињДЖгћПбПавѦѓвТоѓзѪэњѢѡџърЬѡѭуѨтѕчѼѲѕњѺѐѹњѥіѻхѠѳѤ҃ѼпѡѺјҒѥ҉ѾчѶѧѢѷ҂ёҙї҆ѝѯѷѬҀҙҕѯѹҚҚѷѼ҃ѬѦҌҍѭѻҤ҇ѽҬҵ҂ҋҚ҇ѺҘґѺүҚҼҠӄҲѼҵҌҍ", (byte)12, 68
               );
               b[36] = χΠξΦιζΨΣωΦσΨζ.A("ÎĉñċÞÿßĄÔĔûĔĎĚéöÿėÙÙñĜñðòĞĂČħùĜĚúĐýąĪõþġħùĺĨīıěĶüĮĿļĄĴĐŃĂĦĂŃĂęĨĥĆĽĪĭĬģĎĨģķĪğ", (byte)12, 65);
               b[37] = ζβησεθωυγτ.F("ԯԲӼԞԏԿԵԙԽԘԥԅԻԎԋԪԤՐԼԤԠԎԲՌՓՈԷՓԳԝԛՕբբԡԲՂԡԿԢԟՄՙ\u0530", (byte)12, 70);
               break;
            case 2:
               b[0] = ντθΔζβΔζ.B("āÍĊééðþñĎĖĐìÜėĎĐüĠüĘòĞĠÙğýėĕĜĂûü", (byte)12, 66);
            case 3:
            default:
               break;
            case 4:
               b[0] = ΠΛΨοδΩσμνΛγΦφβς.A("øùĎÝĆäÓđĎėùĜîõûĖĎÓāßĤíêë", (byte)12, 65);
         }
      }
   }

   static {
      b();
   }

   private void a(Object var1, SecretKey var2) {
      if (αδοβτΣσΠμμχοΨ.a() != null) {
         Method var10000 = αδοβτΣσΠμμχοΨ.a();
         Object[] var10002 = new Object[eb];
         var10002[ec] = ed;
         var10002[ee] = var2;
         Object var3 = var10000.invoke(null, var10002);
         var10000 = αδοβτΣσΠμμχοΨ.a();
         var10002 = new Object[ef];
         var10002[eg] = eh;
         var10002[ei] = var2;
         Object var4 = var10000.invoke(null, var10002);
         var10000 = αδοβτΣσΠμμχοΨ.b();
         var10002 = new Object[ej];
         var10002[ek] = var3;
         var10002[el] = var4;
         var10000.invoke(var1, var10002);
      } else {
         Method var7 = αδοβτΣσΠμμχοΨ.b();
         Object[] var10 = new Object[em];
         var10[en] = var2;
         var7.invoke(var1, var10);
      }
   }

   protected void a(User var1, Channel var2, WrapperLoginClientEncryptionResponse var3, ΨΓνξςηπΨ var4, boolean var5) {
      if (this.a(var1, var3, var4)) {
         byte[] var6;
         SecretKeySpec var7;
         try {
            var6 = ηΠτΠΩΔγΦρδρ.a(αδοβτΣσΠμμχοΨ.a(this.a), var3.getEncryptedSharedSecret());
            var7 = new SecretKeySpec(var6, c<"㺀">(h, i ^ j));
         } catch (GeneralSecurityException var28) {
            var1.closeConnection();
            return;
         }

         Object var8 = var4.h();
         if (!var5) {
            this.a(var8, var7);
         }

         boolean var9 = κδΔΦφγμδξΨωψο.d.ar();
         στΩξξχκι var10 = var4.a();
         String var11 = var4.ax();
         String var12 = ηΠτΠΩΔγΦρδρ.a(c<"㺃">(k, l ^ m), var6, αδοβτΣσΠμμχοΨ.a(this.a).getPublic());
         νεοξγηζΨρΓΦ var13 = αξλψαςΦυδδΠζυμ.a(αδοβτΣσΠμμχοΨ.a(this.a), var11, var12, null);
         switch (var13.p()) {
            case 0:
               αυμξτυζΩ.j(var1, c<"㺛">(az, ba ^ bb));
               ψΓωτιμωκχψΛΨ.d(c<"㺞">(bc, bd ^ be) + var11 + c<"㺡">(bf & bg, bh) + var13.p() + c<"㺤">(bi, bj ^ bk));
               break;
            case 200:
               φτμΨνψΣτξ var29 = αξλψαςΦυδδΠζυμ.a(var13.V());
               String var15 = var29.getName();
               if (var15 == null) {
                  αυμξτυζΩ.i(var1, c<"㺆">(n, o ^ p) + var15);
                  return;
               }

               UUID var16 = var29.b();
               if (var16 == null) {
                  αυμξτυζΩ.i(var1, c<"㺉">(q, r) + var15);
                  return;
               }

               int var17 = s;
               if (var10 != null && var10.getMojangId() != null && !var16.equals(var10.getMojangId())) {
                  String var32 = c<"㺌">(t, u ^ v);
                  Object[] var34 = new Object[w];
                  var34[x] = var10.getMojangId();
                  var34[y] = var16;
                  var34[z] = var10.i();
                  var34[aa] = var15;
                  ψΓωτιμωκχψΛΨ.b(var32, var34);
                  var10 = null;
                  var17 = ab;
               }

               ςθΓωΩπγΩΩΔβΨω var18 = αδοβτΣσΠμμχοΨ.a(this.a).a();
               στΩξξχκι var19 = var18.a(var15, var16, null, (boolean)ac);
               if (var19 == null) {
                  αυμξτυζΩ.i(var1, εδδΠηδξΛΣχ.a(πωιψγηξΓρφυ.w));
                  return;
               }

               if (var10 == null || var19.r()) {
                  var10 = var19;
               }

               InetAddress var20 = var1.getAddress().getAddress();
               String var21 = var20.getHostAddress();
               String var22 = var9 ? βδΔφμπθθγ.g(var15, (boolean)ae) : var15;
               if (αδοβτΣσΠμμχοΨ.a(this.a).a().b(var1x -> αυμξτυζΩ.i(var1, var1x), var10, var21, ΣλνΨουΩΔοθεβ.I)) {
                  return;
               }

               if (αδοβτΣσΠμμχοΨ.a(this.a).a().a(var1x -> αυμξτυζΩ.i(var1, var1x), var10, var21, ΣλνΨουΩΔοθεβ.Q)) {
                  return;
               }

               if (!var18.a(var10, var16, var15, var21, (boolean)var17)) {
                  αυμξτυζΩ.i(var1, εδδΠηδξΛΣχ.a(πωιψγηξΓρφυ.w));
                  return;
               }

               ΨαχΨχΣλεΠψΦ var23 = αδοβτΣσΠμμχοΨ.a(this.a).b().a(var22);
               if (var23 != null && var23.R()) {
                  var23.a(εδδΠηδξΛΣχ.a(πωιψγηξΓρφυ.Y));
               }

               ωομΦδΔπνΨεβυΓ[] var24 = var29.a();
               ωομΦδΔπνΨεβυΓ var25 = var24.length > 0 ? var24[ah] : null;
               Runnable var26 = μΔψωΠχΛξεκε.a(var8, var22, var10.a(), var25);
               com.nickuc.login.bukkit.ΠιζζοΨκψΠΔπ var27 = new com.nickuc.login.bukkit.ΠιζζοΨκψΠΔπ(
                  var1, var10, var15, var10.a(), (boolean)ai, var26, var2, var25
               );
               var2.attr(com.nickuc.login.bukkit.ΠιζζοΨκψΠΔπ.a).set(var27);
               com.nickuc.login.bukkit.ΠιζζοΨκψΠΔπ.a(var22, var4.ay(), var20, var27);
               if (var5) {
                  var1.receivePacketSilently(var3);
               } else {
                  αδοβτΣσΠμμχοΨ.b(var1, var4.a(), var22);
               }
               break;
            case 204:
               String var30;
               if (!var9 && (var10 == null || !var10.t())) {
                  πωιψγηξΓρφυ var31 = πωιψγηξΓρφυ.ab;
                  Object[] var33 = new Object[al];
                  var33[am] = var11;
                  var30 = εδδΠηδξΛΣχ.a(var31, var33);
               } else {
                  πωιψγηξΓρφυ var10000 = πωιψγηξΓρφυ.aa;
                  Object[] var10001 = new Object[aj];
                  var10001[ak] = var11;
                  var30 = εδδΠηδξΛΣχ.a(var10000, var10001);
               }

               String var14 = var30;
               αυμξτυζΩ.i(var1, var14);
               break;
            case 429:
               αυμξτυζΩ.j(var1, c<"㺏">(an & ao, ap));
               ψΓωτιμωκχψΛΨ.d(c<"㺒">(aq & ar, as) + var11 + c<"㺕">(at, au) + var13.p() + c<"㺘">(av, aw ^ ax));
               break;
            default:
               αυμξτυζΩ.j(var1, c<"㺧">(bm & bn, bo));
               ψΓωτιμωκχψΛΨ.d(c<"㺪">(bp, bq) + var11 + c<"㺭">(br, bs ^ bt) + var13.p() + c<"㺰">(bu, bv));
         }
      }
   }

   private boolean a(User var1, WrapperLoginClientEncryptionResponse var2, ΨΓνξςηπΨ var3) {
      try {
         ψΓςλνΣΠπεοψγΩσπ var4 = var3.a();
         if (var4 != null) {
            SaltSignature var8 = (SaltSignature)var2.getSaltSignature().orElse(null);
            if (var8 == null) {
               αυμξτυζΩ.i(var1, c<"㺀">(bx & by, bz));
               ψΓωτιμωκχψΛΨ.b(c<"㺃">(ca, cb) + var3.ax() + c<"㺆">(cc & cd, ce) + var1.getAddress() + c<"㺉">(cf, cg ^ ch));
               return (boolean)cj;
            } else if (!var4.a(var8.getSignature(), var3.d(), var8.getSalt())) {
               αυμξτυζΩ.i(var1, c<"㺌">(ck, cl ^ cm));
               return (boolean)cn;
            } else {
               return (boolean)co;
            }
         } else {
            byte[] var5 = (byte[])var2.getEncryptedVerifyToken().orElse(null);
            if (var5 == null) {
               αυμξτυζΩ.i(var1, c<"㺏">(cp & cq, cr));
               ψΓωτιμωκχψΛΨ.b(c<"㺒">(cs, ct) + var3.ax() + c<"㺕">(cu, cv ^ cw) + var1.getAddress() + c<"㺘">(cx & cy, cz));
               return (boolean)db;
            } else {
               byte[] var6 = var3.d();
               if (!Arrays.equals(var6, ηΠτΠΩΔγΦρδρ.a(αδοβτΣσΠμμχοΨ.a(this.a), var5))) {
                  αυμξτυζΩ.i(var1, c<"㺛">(dc, dd ^ de));
                  ψΓωτιμωκχψΛΨ.b(
                     c<"㺞">(df, dg ^ dh)
                        + var3.ax()
                        + c<"㺡">(di, dj ^ dk)
                        + var1.getAddress()
                        + c<"㺤">(dl, dm)
                        + Arrays.toString(var6)
                        + c<"㺧">(dn, do ^ dp)
                        + Arrays.toString(var5)
                  );
                  return (boolean)dr;
               } else {
                  return (boolean)ds;
               }
            }
         }
      } catch (Exception var7) {
         αυμξτυζΩ.i(var1, c<"㺪">(dt, du ^ dv));
         if (!(var7 instanceof GeneralSecurityException)) {
            ψΓωτιμωκχψΛΨ.d(c<"㺭">(dw, dx ^ dy));
         }

         return (boolean)ea;
      }
   }

   private static Object c(Lookup var0, String var1, MethodType var2) {
      try {
         return new MutableCallSite(
            var0.findStatic(
                  ψΩδωωΠΔνΠθπμζυ.class,
                  new String(new byte[]{97}, StandardCharsets.UTF_8),
                  MethodType.fromMethodDescriptorString(φΨαξωυθανΣφυκη.C("ϦЈЊϪЎЭХлЧ϶дЪивϻРтсйпйЎ", (byte)3, 67), ψΩδωωΠΔνΠθπμζυ.class.getClassLoader())
               )
               .asType(var2)
         );
      } catch (Exception var4) {
         throw new RuntimeException(
            φδχεθοκψαλΛογλ.F("ԢԯԮӱԱԭԨԱԼԫӸԶԺԳԶԼӾ࢘ࡺࢆ࢜࢝ࡵࡪ\u0894ࡸ\u0891࢚\u0897\u0892ࢢԘ", (byte)3, 70) + var1 + ζοηκορΦνΣθγΩ.B("¿", (byte)3, 66) + var2.toString(),
            var4
         );
      }
   }

   // $VF: Unable to simplify switch on enum
   // Please report this to the Vineflower issue tracker, at https://github.com/Vineflower/vineflower/issues with a copy of the class file (if you have the rights to distribute it!)
   @Override
   public void a(PacketReceiveEvent var1) {
      User var2 = var1.getUser();
      Channel var3 = (Channel)var1.getChannel();
      ΨΓνξςηπΨ var4 = (ΨΓνξςηπΨ)var3.attr(ΨΓνξςηπΨ.e).getAndSet(null);
      if (var4 == null) {
         αυμξτυζΩ.i(var2, c<"㺀">(a, b));
      } else {
         var1.setCancelled((boolean)c);
         WrapperLoginClientEncryptionResponse var5 = new WrapperLoginClientEncryptionResponse(var1);
         boolean var6 = αδοβτΣσΠμμχοΨ.a(this.a).a().getOnlineMode();
         String var7 = var4.ax();
         InetAddress var8 = var2.getAddress().getAddress();
         ΛΨΨγδσκνψνε var9 = νκσΩννοψξ.a(var7, var8);
         switch (ιΛοιψβαηκυΔα.P[var9.ordinal()]) {
            case 1:
            case 2:
               νκσΩννοψξ.a(αδοβτΣσΠμμχοΨ.a(this.a), var7, var8, ΛΨΨγδσκνψνε.a);
            default:
               αδοβτΣσΠμμχοΨ.a(this.a).b((boolean)d).a(() -> {
                  try {
                     if (!var3.isOpen() || !var3.isActive()) {
                        return;
                     }

                     this.a(var2, var3, var5, var4, var6);
                  } catch (Throwable var7x) {
                     αυμξτυζΩ.i(var2, c<"㺀">(eo & ep, eq));
                     ψΓωτιμωκχψΛΨ.c(c<"㺃">(er, es ^ et) + var4.ax() + c<"㺆">(eu & ev, ew), var7x);
                  }
               });
         }
      }
   }
}
